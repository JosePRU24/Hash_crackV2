...
def show_elapsed_time(signum, frame):
    ...
    if elapsed < 60:
       text = f"{elapsed:.2f} seconds"
    elif elapsed < 3600:
       minutes, seconds = divmod(elapsed, 60)
       text = f"{int(minutes)} minutes, {seconds:.2f} seconds"
    else:
       hours, remainder = divmod(elapsed, 3600)
       minutes, seconds = divmod(remainder, 60)
       text = f"{int(hours)} hours, {int(minutes)} minutes, {seconds:.2f} seconds"
...
def call_modules(module_chosen, encoder):
    module = MENU_MODULES.get(module_chosen, None)

    if module_chosen in {'1', '2'} and module:
        time.sleep(1)
        subprocess.run(["clear"], check=True)
        subprocess.run(
            ["python3", f"{HOME}/Hash_crackV2/thirdparty_cracktools/{module}"],
            check=True
        )
        ...
        if sys.platform == "linux":
            ...
            subprocess.run(
                [
                    "proxychains4",
                    "python3",
                    f"{HOME}/Hash_crackV2/config_bundle/{module}"
                ],
                check=True
            )
...
On UserLAND, perform the installation as root, as "sudo" may cause problems.
""".strip())
        ...
        if sys.platform == "linux":
            ...
            subprocess.run(
              ["am", "start", "-a", "android.intent.action.VIEW", "-d", "https://crackstation.net/"],
              check=True
            )
        ...
        if sys.platform == "linux":
            ...
            subprocess.run(
                ["python3", f"{HOME}/Hash_crackV2/config_bundle/{module}"],
                check=True
            )
            ...
            subprocess.run(
                [
                    "python3",
                    f"{HOME}/Hash_crackV2/config_bundle/Pdf_Crack_termux.py"
                ],
                check=True
            )
       ...
       print("[ERROR]: Enter the valid number with the selected module!!")
...
def init_worker(hash_type, target_hash):
    ...
    global hashes_verification_validator, hashes_dic
    global ph, yescrypt_
    ...
    valid_rules = {
        ...
        "26", "62", "36", "63", "lcu", "un", "ln", "sn", "ns", "tn", "rn", "uns", "lns", "cns", "alc", "acu"
    }
    ...
    if hash_type == "sha256crypt":
        ...
        context = CryptContext(schemes=["apr_md5_crypt"])
...
[NOTE] Rules '7', '8', '9', '10', '11', 'HBA', 'lcu', 'un', 'ln', 'sn', 'ns', 'tn', 'rn', 'uns', 'lns', 'cns', 'alc', 'acu' and '00' are not valid in combination and are applied individually
        """
    )
    parser.add_argument(
        "-c", "--cpu-num",
